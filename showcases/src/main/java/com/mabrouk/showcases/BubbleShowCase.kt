...
class BubbleShowCase(builder: BubbleShowCaseBuilder){
    ...
    private val mContentColor: Int? = builder.mContentColor
    ...
    private val mTitleFont: Typeface?=builder.mTitleFont
    private val mSubTitleFont: Typeface?=builder.mSubTitleFont
    private val nextDrawable:Drawable?=builder.nextDrawable
    private val nextButtonText:String?=builder.nextButtonText
    private val nextButtonTextColor:Int?=builder.nextButtonTextColor
    private val nextButtonTextSize:Float?=builder.nextButtonTextSize
    private val nextButtonTextFont:Typeface?=builder.nextButtonTextFont
    private val showNextButton:Boolean?=builder.showNextButton
    private val nextButtonResourceId:Int?=builder.nextButtonResourceId
    private val skipText:String?=builder.skipText
    private val skipTextColor:Int?=builder.skipTextColor
    private val skipTextFont:Typeface?=builder.skipTextFont
    private val skipTextSize:Float?=builder.skipTextSize
    ...
    fun show(overLayColor: Int=R.color.transparent_grey){
        if(mShowOnce != null){
        ...
        backgroundDimLayout = getBackgroundDimLayout(overLayColor)
    ...
    private fun getBackgroundDimLayout(overLayColor:Int=R.color.transparent_grey): RelativeLayout {
        ...
        backgroundLayout.setBackgroundColor(ContextCompat.getColor(mActivity.get()!!, overLayColor))
    ...
    private fun getBubbleMessageViewBuilder(): BubbleMessageView.Builder{
        return BubbleMessageView.Builder()
                ...
                .contentColor(mContentColor)
                .titleTextSize(mTitleTextSize)
                .subtitleTextSize(mSubtitleTextSize)
                .title(mTitle)
                .subtitle(mSubtitle)
                .image(mImage)
                .titleFont(mTitleFont)
                .subTitleFont(mSubTitleFont)
                .nextButtonDrawable(nextDrawable)
                .nextButtonResourceId(nextButtonResourceId)
                .nextButtonText(nextButtonText)
                .nextButtonTextColor(nextButtonTextColor)
                .nextButtonTextFont(nextButtonTextFont)
                .nextButtonTextSize(nextButtonTextSize)
                .showNextButton(showNextButton)
                .closeActionImage(mCloseAction)
                .disableCloseAction(mDisableCloseAction)
                .showSkip(showSkip)
                .skipDrawable(skipDrawable)
                .skipResourceId(skipResourceId)
                .skipText(skipText)
    ...
    private fun takeScreenshotOfLayoutView(targetView: View): Bitmap? {
        ...
        try {
            ...
            bitmap = Bitmap.createBitmap(currentScreenView.drawingCache, 0, 0, targetView.width, targetView.height)
        }
